...
class SyncAssetsThread(QtCore.QThread):
    ...
    def run( self ):
        ...
        for scene in self.data_list:
            ...
            if ret != 0:
                self.update_progress_signal.emit( "Warning: Done syncing assets, but some assets could not be uploaded!'" , 1 , 1 )
                return 1
            else:
                self.update_progress_signal.emit( "Done syncing assets to S3..." , 1 , 1 )
                return 0
